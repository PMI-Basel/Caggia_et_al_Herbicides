...
* **Q1: Are there differences in beta diversity between the herbicide applications?**
...
#all_phy_sub@sam_data$Time <- as.character(all_phy_sub@sam_data$Time)
...
**Q1: Are there differences in beta diversity between the herbicide applications?**  
We could see that there was no effect for bacteria nor for fungi. Herbicide application direct on soil or on weeds was not relevant for the microbial communities. That's why we do not further investigate the mode of application.
...
bdist_ctr_gly_exp1_paov <- adonis2(bdist_ctr_gly_exp1 ~ Herbicide * Application * Time + Plate, 
...
bdist_ctr_tb_exp1_paov <- adonis2(bdist_ctr_tb_exp1 ~ Herbicide * Application * Time + Plate, 
...
bdist_ctr_gly_exp2_paov <- adonis2(bdist_ctr_gly_exp2 ~ Herbicide * Application * Time + Plate, 
...
bdist_ctr_tb_exp2_paov <- adonis2(bdist_ctr_tb_exp2 ~ Herbicide * Application * Time + Plate, 
...
fdist_ctr_gly_exp1_paov <- adonis2(fdist_ctr_gly_exp1 ~ Herbicide * Application * Time + Plate, 
...
fdist_ctr_tb_exp1_paov <- adonis2(fdist_ctr_tb_exp1 ~ Herbicide * Application * Time + Plate, 
...
fdist_ctr_gly_exp2_paov <- adonis2(fdist_ctr_gly_exp2 ~ Herbicide * Application * Time + Plate, 
...
fdist_ctr_tb_exp2_paov <- adonis2(fdist_ctr_tb_exp2 ~ Herbicide * Application * Time + Plate, 
...
bDAT_exp1 <- otu_table(bPHYSEQ_exp1)
bdist_exp1 <- vegdist(t(bDAT_exp1), method="bray")
bdist_exp1_paov <- adonis2(bdist_exp1 ~ Herbicide * Application * Time + Plate, data=bDESIGN_exp1)

score <- paste0("Hc:", 
               format(bdist_exp1_paov$R2[1] * 100, digits=2, nsmall=1),
               "% var, P=",format(bdist_exp1_paov$`Pr(>F)`[1], digits=2))
#plot
phy_sub@sam_data$Time <- as.factor(phy_sub@sam_data$Time)
p0 <- phyloseq::plot_ordination(phy_sub, CAP_bray, color="Herbicide", shape="Time", axes=c(1,2))
p0$layers <- p0$layers[-1]
...
bDAT_exp2 <- otu_table(bPHYSEQ_exp2)
bdist_exp2 <- vegdist(t(bDAT_exp2), method="bray")
bdist_exp2_paov <- adonis2(bdist_exp2 ~ Herbicide * Application * Time + Plate, data=bDESIGN_exp2)
...
fDAT_exp1 <- otu_table(fPHYSEQ_exp1)
fdist_exp1 <- vegdist(t(fDAT_exp1), method="bray")
fdist_exp1_paov <- adonis2(fdist_exp1 ~ Herbicide * Application * Time + Plate, data=fDESIGN_exp1)
...
#plot
phy_sub@sam_data$Time <- as.factor(phy_sub@sam_data$Time)
p0 <- phyloseq::plot_ordination(phy_sub, CAP_bray, color="Herbicide", shape="Time", axes=c(2,3))
p0$layers <- p0$layers[-1]
p0 <- p0 + geom_point(size=3)
...
fDAT_exp2 <- otu_table(fPHYSEQ_exp2)
fdist_exp2 <- vegdist(t(fDAT_exp2), method="bray")
fdist_exp2_paov <- adonis2(fdist_exp2 ~ Herbicide * Application * Time + Plate, data=fDESIGN_exp2)
...
# #gly
# CAP_bray <- ordinate(bPHYSEQ_ctr_gly_exp1, method="CAP", ~ Herbicide + Time + Plate, distance="bray") #CAP
# CAP_bray_var_tbl <- variability_table(CAP_bray) # extract variation details
...
# 
# #tb
# CAP_bray <- ordinate(bPHYSEQ_ctr_tb_exp1, method="CAP", ~ Herbicide + Time + Plate, distance="bray") #CAP
# CAP_bray_var_tbl <- variability_table(CAP_bray) # extract variation details
...
# 
# #gly
# CAP_bray <- ordinate(fPHYSEQ_ctr_gly_exp1, method="CAP", ~ Herbicide + Time + Plate + Plate, distance="bray") #CAP
# CAP_bray_var_tbl <- variability_table(CAP_bray) # extract variation details
...
# 
# #tb
# CAP_bray <- ordinate(fPHYSEQ_ctr_tb_exp1, method="CAP", ~ Herbicide + Time + Plate + Plate, distance="bray") #CAP
# CAP_bray_var_tbl <- variability_table(CAP_bray) # extract variation details
...
# 
# #add legend
# pseudo <- ggplot(bDESIGN[bDESIGN$Time!=0,], aes(x=Time, y=Experiment, color=Herbicide))+geom_point()+scale_color_manual(values=level_cols_Herbicide)+theme_bw()
...
# 
# #gly
# CAP_bray <- ordinate(bPHYSEQ_ctr_gly_exp2, method="CAP", ~ Herbicide + Time + Plate, distance="bray") #CAP
# CAP_bray_var_tbl <- variability_table(CAP_bray) # extract variation details
...
# 
# #tb
# CAP_bray <- ordinate(bPHYSEQ_ctr_tb_exp2, method="CAP", ~ Herbicide + Time + Plate, distance="bray") #CAP
# CAP_bray_var_tbl <- variability_table(CAP_bray) # extract variation details
...
# 
# #gly
# CAP_bray <- ordinate(fPHYSEQ_ctr_gly_exp2, method="CAP", ~ Herbicide + Time + Plate + Plate, distance="bray") #CAP
# CAP_bray_var_tbl <- variability_table(CAP_bray) # extract variation details
...
# 
# #tb
# CAP_bray <- ordinate(fPHYSEQ_ctr_tb_exp2, method="CAP", ~ Herbicide + Time + Plate + Plate, distance="bray") #CAP
# CAP_bray_var_tbl <- variability_table(CAP_bray) # extract variation details
...
**Conclusion:** Applying glyphosate and terbuthylazine altered the microbial beta diversity of bacteria and fungi. But conduction the CAP plots showed that herbicides had a small effect which only explained up to 3% of variation between communities.

\vspace{5mm}

## Conclusion beta diversity

Different applications of herbicide did not alter microbial communities. Herbicides had an effect on bacterial and fungal microbial community compositions. They could explain up to 3% of the microbiome community variation.
