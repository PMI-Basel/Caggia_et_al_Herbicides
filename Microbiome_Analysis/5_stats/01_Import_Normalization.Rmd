...
Two experiments were performed. In experiment 1 (exp1) we kept a constant 60% water holding capacity (WHC) while in experiment 2 (exp2) all pots were flushed (100% WHC) before keeping a constant 60% WHC. In both experiments, we had three Herbicide (Hc) treatments: water control (ctr), glyphosate (gly) and terbuthylazine (tb) which were sprayed on two different targets: Bare soil (S) or weeds (Ca; Chenopodium album). Two weeks after spraying, maize was planted in the soil. Soil samples were collected at: 2W (2 weeks after spraying, maize planting, 7 reps. per treatment) and 5W (5 weeks after spraying, 3 weeks old maize, 7 reps. per treatment).
...
#remove timpeoint t0
DESIGN <- DESIGN[DESIGN$Timepoint != "0",]
...
DESIGN$Spraying <- gsub("cheno", "weeds", DESIGN$Spraying)
DESIGN$Spraying <- factor(DESIGN$Spraying, levels = c("soil","weeds"))
DESIGN$Taxa <- as.factor(DESIGN$Taxa)

#additional columns
DESIGN$Spraying_Timepoint <- paste(DESIGN$Spraying, DESIGN$Timepoint, sep="_")
DESIGN$Spraying_Timepoint <- factor(DESIGN$Spraying_Timepoint,
                                    levels = c("soil_2", "weeds_2",
                                               "soil_5", "weeds_5")) 

DESIGN$group <- paste(DESIGN$Herbicide, DESIGN$Spraying, sep="_")
...
DESIGN$herbicide_timepoint <- as.factor(paste(DESIGN$Herbicide, DESIGN$Timepoint, sep="_"))
DESIGN$herbicide_experiment_timepoint <- as.factor(paste(DESIGN$Herbicide, DESIGN$Experiment, DESIGN$Timepoint, sep="_"))
DESIGN$spraying_experiment_timepoint <- as.factor(paste(DESIGN$spraying, DESIGN$Experiment, DESIGN$Timepoint, sep="_"))
DESIGN$group_experiment_timepoint <- as.factor(paste(DESIGN$group_experiment, DESIGN$Timepoint, sep="_"))
...
DESIGN$cols_Spraying <- NA
DESIGN$cols_Spraying[DESIGN$Spraying=="soil"] <- "firebrick"
DESIGN$cols_Spraying[DESIGN$Spraying=="weeds"] <- "dodgerblue"
...
temp <- data.frame(DESIGN$Spraying, DESIGN$cols_Spraying)
temp <- plyr::ddply(temp, .variables="DESIGN.cols_Spraying", .fun=unique)
level_cols_Spraying <- as.character(temp[,2])
names(level_cols_Spraying) <- temp[,1]
...
df <- data.frame(col_sum=colSums(bDAT),colnames=colnames(bDAT), Spraying=bDESIGN$Spraying, 
                 Herbicide=bDESIGN$Herbicide, group=bDESIGN$group, Experiment=bDESIGN$Experiment, Timepoint=as.factor(bDESIGN$Timepoint),
                 group_experiment_timepoint=bDESIGN$group_experiment_timepoint)

levels(df$Experiment) <- c("Experiment 1", "Experiment 2")
levels(df$Timepoint) <- c("2w", "5w")

ylim2=boxplot.stats(df$col_sum)$stats[c(1, 5)]

seq_nr <- ggplot2::ggplot(data=df, aes(x=Spraying, y=col_sum, fill=Herbicide)) + 
                      geom_boxplot(position=position_dodge2(width=0.75, preserve="single"),outlier.colour = NA) + 
                      geom_jitter(aes(col=Timepoint, group=group_experiment_timepoint), 
...
samples <-  ggplot(df, aes(x=x, y=col_sum, fill=Herbicide, color=Herbicide))+
                ...
                #facet_grid(.~ Spraying, scales='free_x', space="free")
...
## boxplot
df <- data.frame(col_sum=colSums(fDAT),colnames=colnames(fDAT), Spraying=fDESIGN$Spraying, 
                 Herbicide=fDESIGN$Herbicide, group=fDESIGN$group, Experiment=fDESIGN$Experiment, Timepoint=as.factor(fDESIGN$Timepoint),
                 group_experiment_timepoint=fDESIGN$group_experiment_timepoint)
...
seq_nr <- ggplot(data=df, aes(x=Spraying, y=col_sum, fill=Herbicide)) + 
              geom_boxplot(position=position_dodge2(width=0.75, preserve="single"),outlier.colour = NA) + 
              geom_jitter(aes(col=Timepoint, group=group_experiment_timepoint), 
                    size=1, position=position_jitterdodge(jitter.width=0,dodge.width = 0.75))+
              theme_bw() +
...
We discarded eight bacterial samples with too few reads per sample while we kept all fungal samples. We ended up with the following number of samples per treatment for the analysis. The samples were labeled as "Herbicide_Spraying_Experiment_Timepoint":

```{r sample size rare, warning=F, echo=F}

#final number of samples
btable <- table(bDESIGN$group_experiment_timepoint)
ftable <- table(fDESIGN$group_experiment_timepoint)
...
saveRDS(level_cols_Spraying, "level_cols_Spraying.RDS")
