...
for (run in runs) {
  ...
  path <- paste("../2_datafungi/", run, sep="")
...
saveRDS(taxa, paste(temp, "taxa.RDS"))
...
if(output){
  ...
  out <- c("../../4_output/fungi_ASV/")
  
  #quality profiles unfiltered and untrimmed
  pdf(paste(out,"fungi_reads_quality_unfilt_untrim.pdf",sep=""))
  ...
  pdf(paste(out,"fungi_reads_quality_filt_trim.pdf",sep=""))
  for (i in 1:length(sample.names)) {
  ...
  pdf(paste(out,"fungi_error_rate.pdf",sep=""))
  ...
  write.table(seqtab.nochim, paste(out, "fungi_ASV.tab", sep=""), sep="\t")
  write.table(taxa, paste(out, "fungi_taxa.tab", sep=""), sep="\t")
  write.table(ASV_seq, paste(out, "fungi_sequences.fasta", sep=""), quote=F, row.names = FALSE, col.names = FALSE)
  
  
  #number of reads
  write.xlsx(track, paste(out, "fungi_track.xlsx", sep=""))
}